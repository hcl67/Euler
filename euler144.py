...
n = 1
plist = [p1]
while not (abs(p1[0])<0.01 and p1[1] > 0):
    kn = p1[1]/p1[0]/4    #法线斜率
    ln = getLine4kp(kn, p1)  #法线方程
    pt = getPoint4prl(p0, ln) #出射方向
    lo = getLine4pp(p1,pt)      #出射线
    np0,np1 = getPoint4lxe(lo, e)  #出射线与椭圆的2个交点
    if getPointDist(np0, p1) < eps:
        p2 = np1
    else:
        p2 = np0
    p0 = p1
